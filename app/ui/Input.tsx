  ...
  return (
    <>
      <div className="flex justify-between flex-col gap-10 sm:flex-row">
        <div className="px-5 py-3 h-fit flex gap-5 dark:bg-dark_mode_elements bg-white w-full sm:w-[400px] rounded-md">
          <span>
            <svg
              viewBox="0 0 24 24"
        ...
        <div className="relative flex flex-col gap-5 w-fit sm:min-w-[220px]">
          <div className="h-[54px] p-3 bg-dark_mode_white dark:bg-dark_mode_elements rounded-md flex gap-10 w-full items-center justify-between">
            ...
            <span role="button" onClick={() => setDropdown(!dropdown)}>
              <svg
                viewBox="0 0 24 24"
                height={25}
          ...
          <div
            className={`absolute w-full top-full mt-5 px-1 py-3 shadow-country flex bg-white dark:*:text-dark_mode_white dark:bg-dark_mode_elements rounded-md flex-col ${
              !dropdown && "hidden"
            } transition`}
          >
            {region.map((region: string, index: number) => {
              return (
                <b
                  onClick={() => {
                    setSelected(region);
                  ...
                  key={index}
                  className=" p-3 rounded-md dark:hover:bg-dark_mode_bg hover:bg-light_mode_input cursor-pointer hover:text-white capitalize text-light_mode_input font-medium"
                >
                  {region}
                </b>
              );
